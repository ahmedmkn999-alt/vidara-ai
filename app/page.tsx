...
export default function HomePage() {
  const plans = [
    ...
    { name: "الملكية", price: "700ج", limit: "لا نهائي (سنوي)", color: "from-yellow-500 to-orange-500" }
  ];

  return (
    <div className="text-white min-h-screen">
      <nav className="p-6 flex justify-between items-center sticky top-0 bg-[#0b0f19]/90 backdrop-blur-md z-50">
        <div className="text-3xl font-black bg-gradient-to-r from-purple-400 to-pink-500 bg-clip-text text-transparent">VIDARA</div>
        <Link href="/api/auth/signin" className="bg-gradient-to-r from-purple-600 to-blue-600 px-8 py-3 rounded-full font-bold shadow-xl">دخول / سجل بجوجل</Link>
      </nav>

      <section className="py-20 text-center px-4">
        <h1 className="text-5xl font-black mb-10 italic text-purple-400 italic">إزاي تشغل الموقع؟ 🎬</h1>
      ...
      <section className="py-16 px-6 max-w-7xl mx-auto text-right font-bold">
        <h2 className="text-center text-4xl font-bold mb-16 italic text-purple-400">باقات الاشتراك (PRO)</h2>
        <div className="grid md:grid-cols-4 gap-8 text-center">
          {plans.map((plan, i) => (
            <div key={i} className="bg-[#161b2a] rounded-[35px] p-8 border border-gray-800 hover:scale-105 transition-transform relative overflow-hidden group">
              ...
              <p className="text-gray-400 mb-8">{plan.limit}</p>
